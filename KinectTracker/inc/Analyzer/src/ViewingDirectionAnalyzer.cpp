...
void ViewingDirectionHistogramAnalyzer::update( cv::Mat image )
{
    ...
    normalize( b_hist, b_hist, 0, image.rows, NORM_MINMAX, -1, Mat() );
    normalize( g_hist, g_hist, 0, image.rows, NORM_MINMAX, -1, Mat() );
    normalize( r_hist, r_hist, 0, image.rows, NORM_MINMAX, -1, Mat() );
}
